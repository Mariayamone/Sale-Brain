        ...
        <section>
          ...
          {shop.products.length > visibleProducts.length && (
            <p className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500">
              Showing {visibleProducts.length} of {shop.products.length} products here. Ask the assistant for a specific product, budget, or category.
            </p>
          )}
        </section>
        ...
        <section className="mt-6 rounded-3xl bg-slate-950 px-5 py-5 text-slate-100 shadow-[0_12px_36px_rgba(15,23,42,0.16)]">
          ...
          <div className="mt-4 space-y-4 text-sm">
            <div>
              ...
              <p className="mt-1 leading-6 text-slate-100">{shop.paymentInfo || "Ask for payment details in chat."}</p>
            </div>
            <div>
          ...
          <div className="absolute bottom-0 left-0 right-0 flex max-h-[86dvh] min-h-[58dvh] flex-col overflow-hidden rounded-t-[30px] border border-white/80 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              ...
              <button
                type="button"
                onClick={() => setInfoOpen(false)}
                className="rounded-full border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-700 cursor-pointer"
              >
                Close
              </button>
            </div>
